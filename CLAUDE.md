...
- **2026-07-14 (auth):** Password minimum is 8 (`config.toml` `minimum_password_length`), matched by the client forms. Forgot-password flow: `/forgot-password` (public, `resetPasswordForEmail`) → recovery email (`supabase/templates/recovery.html`, `type=recovery`) → `/auth/confirm` → `/reset-password`. Invite + reset share `src/components/set-password-form.tsx`. New public route `/forgot-password` added to `src/lib/auth/routes.ts`.
